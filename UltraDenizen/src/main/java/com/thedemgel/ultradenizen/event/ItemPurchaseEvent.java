...
public class ItemPurchaseEvent implements SmartEvent, Listener {

    @Override
    public boolean shouldInitialize(Set<String> events) {
        for (String event : events) {
            Matcher m = Pattern.compile("on item purchase", Pattern.CASE_INSENSITIVE).matcher(event);
    ...
    public void _initialize() {
        ...
        dB.log("Loaded item purchase SmartEvent.");
    }
    ...
    // -->
    @EventHandler
    public void onPlayerPurchase(PlayerPurchaseFromShopEvent event) {
        ...
        String determination = EventManager.doEvents(Arrays.asList("item purchase"), null, event.getPlayer(), context).toUpperCase();
